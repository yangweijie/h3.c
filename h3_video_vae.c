...
/* Async VAE block prefetch was removed: its worker thread loaded block N+1 on
 * the shared vae->gpu while the main thread held an open command buffer, which
 * raced and crashed (objc_retain of a dangling pointer). Decode is now fully
 * serial (load -> run -> free per block); correct and only marginally slower
 * for the VAE stage. See the decode loop below. */
